...
class T5EncoderTrainer(Trainer):
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            loss, logits, labels, input_ids = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
        ...
        if self.compute_metrics is not None and all_preds is not None and all_labels is not None:
            metrics = self.compute_metrics(predictions=all_preds, labels=all_labels, input_ids=all_input_ids)
        else:
            metrics = {}
    ...
    def prediction_step(
            ...
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    with self.compute_loss_context_manager():
                        ...
                        loss, outputs, input_ids = self.compute_loss(model, inputs, return_outputs=True)
        ...
        return (loss, logits, labels, input_ids)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        outputs, input_ids = model(**inputs)
        ...
        assert return_outputs, "return_outputs must be set to True to return a tuple"
        
        return (loss, outputs, input_ids) if return_outputs else loss
