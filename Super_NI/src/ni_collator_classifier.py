...
class DataCollatorForNI:
    ...
    def __call__(self, batch, return_tensors=None):
        ...
        sources = []
        id_list = [] # list of id, used for determining different instance
        ...
        for instance in batch:
            ...
            if not task_input[-1] in string.punctuation:
                task_input += "."
            task_input += "\n"
            task_input += f"Output: {instance['Instance']['candidate_output']}"
            if not task_input[-1] in string.punctuation:
                task_input += "."
            task_input += "\n"
            task_input += "Is this example correct?"
            ...
            candidate_list.append(instance["Instance"]["candidate_output"])
        ...
        if "label" in batch[0]["Instance"] and batch[0]["Instance"]["label"]:
            model_inputs["labels"] = [int(ex["Instance"]["label"]) for ex in batch]
            # convert to torch.longtensor
            model_inputs["labels"] = torch.tensor(model_inputs["labels"], dtype=torch.long)
        else:
            model_inputs["labels"] = None

        model_inputs["id"] = id_list
        model_inputs["candidates"] = candidate_list
        assert len(model_inputs["id"]) == model_inputs["labels"].shape[0] == len(model_inputs["candidates"]) 
