    ...
    def evaluation_loop(
        ...
        if self.compute_metrics is not None and all_preds is not None and all_labels is not None and all_candidates is not None:
            ...
            for ii, id in enumerate(all_ids):
                # prefix = id.split('_candidate_')[0] # Get the prefix of the current id
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    ...
                    assert isinstance(outputs, dict), "in this experiment, we assume that the model outputs a dict"
                    # ids = outputs.pop("ids")
