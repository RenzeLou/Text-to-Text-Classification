...
"""
This scripts was only designed for loading the trained generator and evaluating it on the generation tasks.
For example, load `./output_generator/t5-small-mix_gen_true` and save the eval results to `./output_generator/t5-small-mix_gen_true/eval_on_gen`
"""
...
class DataTrainingArguments:
    ...
    train_mix_gen: bool = field(
        default=False, metadata={"help": "Whether to use generation tasks for training."}
    )
...
def main():
    ...
    # check if doing training
    assert not training_args.do_train, "This script is only for evaluation on generation tasks."

    # Get the NaturalInstructions dataset
    raw_datasets = load_dataset(
        "src/ni_dataset_eval_on_gen.py", 
